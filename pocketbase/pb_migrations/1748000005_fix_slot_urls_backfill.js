...
// 1748000004 added the columns but its backfill loop used `date = {:date}`,
// which compares "2026-06-03" against the stored datetime "2026-06-03 00:00:00.000Z"
// and matches zero rows. This re-runs the backfill with `date ~ {:date}` (prefix
// match), which is the same operator the days subscription in useStagData uses.
// Idempotent \u2014 skips rows whose URL fields already match.
...
migrate((db) => {
  ...
  console.log("[1748000005_fix_slot_urls_backfill] backfilled " + updated + " slots");
}, (db) => {
  // No-op: leaving URLs in place on rollback. The columns are dropped by
  // rolling back 1748000004 if you really need to.
});
